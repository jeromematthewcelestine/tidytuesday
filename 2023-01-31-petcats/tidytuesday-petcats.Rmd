...
My understanding is that the data contains records from June-August 2017 of observations of tags that were attached to a number of pet cats in the United Kingdom. Each record contains the coordinates and timetamp of a location "event". As yet, I don't know what triggers an "event" but for now we will take the events as given.

First let's make a simple map of the cat location events. The plot below shows each tagged cat's median location in the dataset, after removing observations that have been classified as outliers. The basemap used here is from [Stamen](https://stamen.com/).
